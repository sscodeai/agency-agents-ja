...
convert_antigravity() {
  ...
  # Antigravity Agent-Skills SKILL.md: standard name + description frontmatter
  # and the persona body. Installs globally to ~/.gemini/config/skills/ or
...
convert_zcode() {
  ...
  # ZCode agent format: .md with YAML frontmatter in .zcode/agents/ (project)
  # or ~/.zcode/agents/ (user). Same plain markdown shape as Qwen today.
  if [[ -n "$tools" ]]; then
    cat > "$outfile" <<HEREDOC
---
name: ${slug}
