...
output="$($INSTALLER --tool claude-code,claude-code --agent 'Developer Tooling Engineer' --dry-run 2>&1)"
[[ "$output" == *"Tools:   claude-code"* ]] || {
  printf 'Repeated --tool values were not deduplicated in the dry-run plan:\n%s\n' "$output" >&2
  exit 1
}

echo "PASS: install.sh rejects unknown agent selections and deduplicates repeated tools"
